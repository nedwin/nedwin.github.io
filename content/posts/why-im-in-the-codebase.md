...
description = "I'm a non-engineer CEO and I've started shipping features myself, to feel every brake between 'the code works' and 'customers have it,' then write each one down with a fix."
...
The feature is tiny. What's interesting is what happened after it merged, which was nothing. The code works. It's deployed. Customers could be using it tomorrow. But it's still not enabled in every account, it's not in the changelog, nobody's announced it, and I didn't actually know what to do next. That gap, between "the code works" and "customers have it," is the reason I'm in the codebase at all.
...
I wrote a while back that you have to [touch AI psychosis](https://neddwyer.com/you-have-to-touch-ai-psychosis/) to get any good at this. You go too far in, watch it not quite work, and drag yourself back one step. This post is me doing that, and what you learn is exactly where the model stops being trustworthy, which is the only thing that lets you build sane rules about where it gets to run.

## The guardrails

So I gave myself rules, and I think of the work in three tiers.

**Big systems. Spike, don't ship.** Scheduling, highlight reels, the core stuff. I'm not going to vibe-code a new scheduler; someone would rightly yell at me. But I can spike it, build the mocks, work out how it should go, and hand it to someone who can actually carry it. I'm doing exactly that with scheduling right now, jamming with one of our engineers who's already poked holes in my version. Good, that's what the spike was for.
